...
export const Copilot: React.FC<CopilotProps> = ({ inquiry }: CopilotProps) => {
  ...
  if (error) {
    return (
      <Card className="p-4 w-full flex justify-between items-center bg-background border-border">
        <div className="flex items-center space-x-2">
          <Sparkles className="w-4 h-4 text-foreground" />
          <h5 className="text-foreground text-xs truncate">
            {`error: ${error}`}
          </h5>
        </div>
      </Card>
  ...
  if (completed) {
    return (
      <Card className="p-3 md:p-4 w-full flex justify-between items-center bg-background border-border">
        <div className="flex items-center space-x-2 flex-1 min-w-0">
          <IconLogo className="w-4 h-4 flex-shrink-0 text-foreground" />
          <h5 className="text-foreground text-xs truncate">
            {updatedQuery()}
          </h5>
        </div>
    ...
    return (
      <Card className="p-4 rounded-lg w-full mx-auto bg-background border-border">
        <div className="flex items-center mb-4">
          <IconLogo
            className={cn('w-4 h-4 flex-shrink-0 text-foreground', { 'animate-spin': pending })}
          />
        ...
        <form onSubmit={onFormSubmit}>
          <div className="flex flex-wrap justify-start mb-4">
            {data?.options?.map((option, index) => (
              ...
              >
                ...
                />
                <label
                  className="text-sm whitespace-nowrap pr-4 text-foreground"
          ...
          {data?.allowsInput && (
            <div className="mb-6 flex flex-col space-y-2 text-sm">
              <label className="text-foreground" htmlFor="query">
                {data?.inputLabel}
              </label>
              <Input
                type="text"
